...
def newton_cai_bien_3an(f1_str, f2_str, f3_str, x0, n, filename="kq_newton_cai_bien_3an.txt"):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        log("="*110)
        log(" PHƯƠNG PHÁP NEWTON CẢI BIÊN 3 ẨN - TỰ ĐỘNG TÍNH ĐẠO HÀM")
        log("="*110)
        ...
        for i in range(n + 1):
            ...
            history.append((i, x[0], x[1], x[2], norm_F))
            ...
            log(f"    J⁻¹(X⁰) · F(X⁽{i}⁾) = [{trunc8(tich_phan[0])}, {trunc8(tich_phan[1])}, {trunc8(tich_phan[2])}]ᵀ")
            log(f"    dX⁽{i}⁾ = [{trunc8(-tich_phan[0])}, {trunc8(-tich_phan[1])}, {trunc8(-tich_phan[2])}]ᵀ")
            log(f"    ‖dX⁽{i}⁾‖∞ = {dx:.6e}")
            ...
            log(f"    X⁽{i+1}⁾ = X⁽{i}⁾ + dX⁽{i}⁾ = [{trunc8(x_new[0])}, {trunc8(x_new[1])}, {trunc8(x_new[2])}]ᵀ")
        ...
        log("")
        log("="*110)
        log(" BẢNG TỔNG HỢP TIẾN TRÌNH LẶP NEWTON CẢI BIÊN 3 ẨN")
        log("="*110)
        log(f"{'k':<4} | {'x₁⁽ᵏ⁾':<14} | {'x₂⁽ᵏ⁾':<14} | {'x₃⁽ᵏ⁾':<14} | {'‖F‖₂':<14} | {'Nhận xét':<25}")
        log("-"*110)
        for row in history:
            k_v, x1_v, x2_v, x3_v, f_v = row
            if f_v < 1e-10:
                ...
                nx = "Cần lặp thêm"
            log(f"{k_v:<4} | {trunc8(x1_v):<14} | {trunc8(x2_v):<14} | {trunc8(x3_v):<14} | {f_v:<14.6e} | {nx:<25}")
        log("="*110)
...
if __name__ == "__main__":
    print(">>> PHƯƠNG PHÁP NEWTON CẢI BIÊN 3 ẨN - NHẬP PHƯƠNG TRÌNH TỪ BÀN PHÍM")
    ...
    newton_cai_bien_3an(eq1, eq2, eq3, x0, num_iter)
